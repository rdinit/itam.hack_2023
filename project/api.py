...
@api.route('/api/tags', methods=['PUT'])
def add_tags():
    if not request.json:
...
@api.route('/api/subtags', methods=['PUT'])
def add_subtags():
    if not request.json:
        return jsonify({'error': 'Empty request'})
...
@api.route('api/subtags/<int:subtag_id>', methods=['DELETE'])
...
@api.route('api/user/friend', methods=['POST'])
def add_friend():
    if not request.json:
        return jsonify({'error': 'Empty request'})
...
@api.route('api/user/friend', methods=['DELETE'])
def delete_friend():
    if not request.json:
        return jsonify({'error': 'Empty request'})
    user_id = request.json['id']
...
@api.route('api/user/bio', methods=['PUT'])
def update_bio():
    if not request.json:
        return jsonify({'error': 'Empty request'})
